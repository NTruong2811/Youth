...
if (isset($_REQUEST['direct']) && empty($_REQUEST['cate_id'])) {
    ...
    order_update($_REQUEST['confirm_id']);
    header('Location: ?order_infor');
} else if (isset($_REQUEST['order_infor'])) {
    $main_title_sub_banner = "ĐƠN HÀNG CỦA BẠN";
    $sub_title = "ĐƠN HÀNG";

    $list_delivery = order_check_delivery(0, $_SESSION['isLogin']['user_id']);
    $list_deliveried = order_check_delivery(1, $_SESSION['isLogin']['user_id']);
