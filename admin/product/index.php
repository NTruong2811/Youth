...
if (isset($_REQUEST['controller']) && empty($_REQUEST['direct'])) {
    ...
    if ($_REQUEST['direct'] == "add") {
        if (isset($_REQUEST['btn_add'])) {
            extract($_REQUEST);
            if (
                $product_name != "" && $price != "" && $_FILES['image']['size'] > 0 && $description != "" && $discount != "" && isset($_POST['model']) && $quantity != ""
                && $status != "" && $cate_id != ""
            ) {
                ...
                if ($image != "") {
                    product_new($product_name, $price, $image, $description, $discount, $model, $quantity, $status, $create_at, $cate_id);
